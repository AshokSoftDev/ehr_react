...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const renderNavItem = (route: RouteConfig, level: number = 0) => {
    ...
    const isAnyChildActive = (route: RouteConfig): boolean => {
      ...
      return route.children.some(child => location.pathname.startsWith(child.path));
    };
  ...
  return (
    <>
      {/* Mobile Overlay */}
      {isMobile && isExpanded && (
        <div className="fixed inset-0 bg-background/50 backdrop-blur-sm z-80 md:hidden" style={{ top: '3.5rem' }} />
      )}
