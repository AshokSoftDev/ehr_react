        ...
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
                <Button
                    variant="outline"
                    size="sm"
                    className="h-7 w-full justify-start gap-2 text-muted-foreground border-dashed hover:border-primary/50 hover:bg-primary/5"
                >
                    <Search className="h-3 w-3" />
                    <span className="text-[10px]">Search drug...</span>
                </Button>
            </PopoverTrigger>
                                    ...
                                    >
                                        <div className="flex items-center gap-2">
                                            <div className="h-5 w-5 rounded bg-emerald-100 dark:bg-emerald-900/50 flex items-center justify-center">
                                                <Pill className="h-2.5 w-2.5 text-emerald-600" />
                                            </div>
                                            <div>
                                                <p className="font-medium text-xs">{drug.drug_name}</p>
...
export function InlinePrescriptionAccordion({
    ...
    useEffect(() => {
        if (existingPrescriptions.length > 0 && !isInitialized) {
            const existingRows: PrescriptionRow[] = existingPrescriptions.map((rx) => ({
                id: generateId(),
                ...
                morning_bf: rx.morning_bf || false,
                morning_af: rx.morning_af || false,
                noon_bf: rx.noon_bf || false,
                noon_af: rx.noon_af || false,
                evening_bf: rx.evening_bf || false,
                evening_af: rx.evening_af || false,
                night_bf: rx.night_bf || false,
                night_af: rx.night_af || false,
            }));
    ...
    return (
        <div className="space-y-3">
            {/* Template Bar */}
            <div className="flex items-center gap-2 p-2 rounded-lg bg-gradient-to-r from-amber-100/50 to-orange-100/50 dark:from-amber-900/20 dark:to-orange-900/20 border border-amber-200/30 dark:border-amber-700/30">
                <div className="h-6 w-6 rounded-full bg-amber-200 dark:bg-amber-800/50 flex items-center justify-center">
                    <BookTemplate className="h-3 w-3 text-amber-700 dark:text-amber-300" />
                </div>
                <Popover open={templateSearchOpen} onOpenChange={setTemplateSearchOpen}>
                    <PopoverTrigger asChild>
                        <Button
                            variant="outline"
                            size="sm"
                            className="h-7 flex-1 max-w-[180px] justify-start gap-1.5 border-amber-300 dark:border-amber-700 bg-white dark:bg-background hover:bg-amber-50 dark:hover:bg-amber-950/50"
                        >
                            <Search className="h-3 w-3 text-amber-600" />
                            <span className="text-muted-foreground text-[10px]">Load template...</span>
                        </Button>
                    </PopoverTrigger>
                                        ...
                                        {groupedTemplates.map((group) => (
                                            ...
                                            >
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        <p className="font-medium text-xs">{group.template_name}</p>
                                                        <p className="text-[10px] text-muted-foreground">
                ...
                {hasSelectedRows && (
                    <>
                        <Input
                            ...
                            className="h-7 text-[10px] w-28 border-emerald-300 dark:border-emerald-700"
                        />
                        <Button
                            size="sm"
                        ...
                        >
                            {bulkCreateTemplate.isPending ? <Loader2 className="h-3 w-3 animate-spin" /> : <Save className="h-3 w-3" />}
                        </Button>
                    </>
                )}
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-muted/50">
                            <tr>
                                ...
                                <th className="px-2 py-2 text-center font-medium" colSpan={2}>Noon</th>
                            ...
                            <tr className="bg-muted/30">
                                ...
                                <th></th>
                                <th></th>
                                <th className="px-1 py-1 text-center text-xs text-muted-foreground">AF</th>
                                <th className="px-1 py-1 text-center text-xs text-muted-foreground">BF</th>
                                <th className="px-1 py-1 text-center text-xs text-muted-foreground">AF</th>
                                <th className="px-1 py-1 text-center text-xs text-muted-foreground">BF</th>
                                <th className="px-1 py-1 text-center text-xs text-muted-foreground">AF</th>
                                <th className="px-1 py-1 text-center text-xs text-muted-foreground">BF</th>
                                    ...
                                    <tr key={row.id} className="border-t hover:bg-muted/20">
                                        ...
                                        <td className="px-2 py-2 min-w-[180px]">
                                            {hasDrug ? (
                                                <div className="flex flex-col gap-1">
                                                    <div className="flex items-center gap-2">
                                                        <Badge variant="secondary" className="px-2 py-0.5 text-xs bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-950/40 dark:to-teal-950/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                                                    ...
                                                    <p className="text-xs text-muted-foreground pl-1">
                                                        {row.drug_generic} • {row.drug_dosage}{row.drug_measure}
                                                    </p>
                                                </div>
                                            ) : (
                                                <DrugSearchCell
                                        ...
                                        <td className="px-2 py-2">
                                            <Input
                                                ...
                                                placeholder="1"
                                                className="h-8 text-sm w-14 px-2"
                                            />
                                        </td>
                                        {/* Morning */}
                                        <td className="px-1 py-2 text-center">
                                            <Checkbox checked={row.morning_af} onCheckedChange={(c) => updateRow(row.id, { morning_af: !!c })} className="h-4 w-4" />
                                        </td>
                                        <td className="px-1 py-2 text-center">
                                            <Checkbox checked={row.morning_bf} onCheckedChange={(c) => updateRow(row.id, { morning_bf: !!c })} className="h-4 w-4" />
                                        </td>
                                        {/* Noon */}
                                        <td className="px-1 py-2 text-center">
                                            <Checkbox checked={row.noon_af} onCheckedChange={(c) => updateRow(row.id, { noon_af: !!c })} className="h-4 w-4" />
                                        </td>
                                        <td className="px-1 py-2 text-center">
                                            <Checkbox checked={row.noon_bf} onCheckedChange={(c) => updateRow(row.id, { noon_bf: !!c })} className="h-4 w-4" />
                                        </td>
                                        {/* Night */}
                                        <td className="px-1 py-2 text-center">
                                            <Checkbox checked={row.night_af} onCheckedChange={(c) => updateRow(row.id, { night_af: !!c })} className="h-4 w-4" />
                                        </td>
                                        <td className="px-1 py-2 text-center">
                                            <Checkbox checked={row.night_bf} onCheckedChange={(c) => updateRow(row.id, { night_bf: !!c })} className="h-4 w-4" />
                                        </td>
                                        <td className="px-2 py-2">
                                            <Button
                                                variant="ghost"
...
export default InlinePrescriptionAccordion;
