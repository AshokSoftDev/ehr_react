...
export function SocialList({ items, isLoading, onEdit, onDelete }: SocialListProps) {
  if (isLoading) {
    return (
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground border rounded-md border-dashed">
        ...
        <h3 className="font-semibold text-lg mb-1">No social history items found</h3>
        <p className="text-sm">Add a social history item to get started</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-2 pb-4">
      <div className="bg-card overflow-hidden">
        <div className="divide-y divide-border">
          {items.map((item) => (
            <div key={item.social_master_id} className="group/item relative flex flex-col sm:flex-row sm:items-center justify-between p-2 hover:bg-muted/30 transition-colors gap-4">
              <div className="flex-1 flex flex-col gap-2">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0 flex items-center gap-3">
                    <h3 className="font-semibold text-base truncate">{item.socialName}</h3>
                    <div className="flex items-center gap-1.5">
                      <Badge variant="outline" className="text-[10px] bg-slate-50 border-slate-200 text-slate-700 h-5 px-1.5 font-normal">
                        Option 1: <span className="font-semibold ml-1">{item.option1}</span>
                      </Badge>
                      <Badge variant="outline" className="text-[10px] bg-slate-50 border-slate-200 text-slate-700 h-5 px-1.5 font-normal">
                        Option 2: <span className="font-semibold ml-1">{item.option2}</span>
                      </Badge>
                    </div>
                    {item.status === 1 ? (
