import { Pencil, Trash2, HeartPulse } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import type { PmhItem } from "../types/pmh.types";

interface PmhListProps {
  pmhs: PmhItem[];
  isLoading: boolean;
  onEdit: (pmh: PmhItem) => void;
  onDelete: (pmh: PmhItem) => void;
}

export function PmhList({ pmhs, isLoading, onEdit, onDelete }: PmhListProps) {
  if (isLoading) {
    return (
  ...
  if (pmhs.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground border rounded-md border-dashed">
        ...
        <h3 className="font-semibold text-lg mb-1">No PMH conditions found</h3>
        <p className="text-sm">Add a condition to get started</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-2 pb-4">
      <div className="bg-card overflow-hidden">
        <div className="divide-y divide-border">
          {pmhs.map((pmh) => (
            <div key={pmh.pmh_id} className="group/item relative flex flex-col sm:flex-row sm:items-center justify-between p-2 hover:bg-muted/30 transition-colors gap-4">
              <div className="flex-1 flex flex-col gap-2">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0 flex items-center gap-3">
                    <h3 className="font-semibold text-base truncate">{pmh.conditionName}</h3>
                    {pmh.status === 1 ? (
