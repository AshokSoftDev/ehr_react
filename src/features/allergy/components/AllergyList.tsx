import { Pencil, Trash2, ShieldAlert, Activity } from "lucide-react";
  ...
  if (allergies.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground border rounded-md border-dashed">
        ...
        <h3 className="font-semibold text-lg mb-1">No allergies found</h3>
        <p className="text-sm">Add an allergy to get started</p>
      </div>
    );
  }
      ...
      <div className="bg-card overflow-hidden">
        <div className="divide-y divide-border">
          {allergies.map((allergy) => (
            <div key={allergy.allergy_id} className="group/item relative flex flex-col sm:flex-row sm:items-center justify-between p-2 hover:bg-muted/30 transition-colors gap-4">
